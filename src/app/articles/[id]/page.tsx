...
export default function Page({ params }: { params: { id: string } }) {
  ...
  const { data: article, error, isLoading } = useGetData(`article/${params.id}`);
  ...
  const { id, title, date, author, text, seo_description, keyword } = article
  let content
      ...
      <section id="article" className="mb-[72px] t:mb-[100px] d:mb-[120px]">
        <div className="container flex flex-col items-center ">
          <div className='flex font-medium gap-x-3 w-full'>
            <Link href="/" title="Головна" className='cursor-pointer '> Головна  </Link>
            <Icon name='/assets/icons/small.svg' id='chevronUp'
              className={cn('w-5 h-5 shrink-0 rotate-90')}
            />
            <Link href="/articles" title="Статті" className='  '> Статті  </Link>
            <Icon name='/assets/icons/small.svg' id='chevronUp'
              className={cn('w-5 h-5 shrink-0 rotate-90')}
            />
            <p className="">{title}</p>
            ...
            <>
              ...
              <div className={cn("w-full h-full flex flex-col rounded-[5px] ")}>
                <Image
                  alt={`фото${title} `}
                  // src={image}
                  src={`https://star-talk.foradmin.pp.ua/api/v1/article/image/${id}`}
                  height={555}
                  width={1200}
                ...
                <div className="flex flex-col t:flex-row gap-y-8  justify-between leading-[1.5] mt-8 t:mt-12 w-full">
                  ...
                  <div className="flex  items-center gap-x-5 leading-[1.5] ">
                    ...
                    <Link href="#instagram" title="instagram" className='flex shrink-0 icon'>
                      <Icon name='/assets/icons/social.svg' id='instagram'
                        className={cn('w-8 h-8 shrink-0')}
                      />
                    </Link>
                    <Link href="#facebook" title="facebook" className='flex shrink-0 icon'>
                    ...
                    <Link href="#telegram" title="telegram" className='flex shrink-0 icon'>
